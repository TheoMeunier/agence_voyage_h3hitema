...
function validName(string|null $name):bool{
    $IsValid = false;
...
function validEmail(string|null $email):bool{
    $IsValid = false;
...
function checkName(string $new_name, string $old_name, int $edit_id):void
{
    if(isNotBlank($new_name)){
        if($old_name != $new_name && Exist('USER', 'name', $new_name)){
            alert('error', 'Cet identifiant est déjà utilisé');
        }else if($old_name != $new_name && !Exist('USER', 'name', $new_name)){
            $data = [
                'name' => $new_name
            ];
...
function checkEmail(string $new_email, string $old_email, int $edit_id):void
{
    if(isNotBlank($new_email)){
        if($old_email != $new_email && Exist('USER', 'email', $new_email)){
            alert('error', 'Cet email est déjà utilisé');
        }else if($old_email != $new_email && !Exist('USER', 'email', $new_email)){
            $data = [
                'email' => $new_email
            ];
            update('USER', $data, $edit_id);
